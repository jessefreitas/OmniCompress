...
enum Cmd {
    ...
    /// Benchmark compression by content type over a directory of raw payload files.
    ///
    /// Each file's raw content is classified (json/code/log/prose/...) and
    /// compressed; the report shows compression ratio per content kind, overall,
    /// and a round-trip fidelity check.
    Bench {
        /// Directory containing raw payload files (any extension).
        dir: String,
    },
}
...
fn run_bench(dir: &str) {
    ...
    let report = omnicompress_core::bench::bench(payloads);
    let by_kind: serde_json::Map<String, serde_json::Value> = report
        .by_kind
        .iter()
        .map(|(k, s)| {
            (
